 ...
 * @brief SIMUL/MEGA EXT subsystem low level driver source.
 *
 * @addtogroup EXT
 * @{
 */
...
#if SIMUL_EXT_USE_INT0 || defined(__DOXYGEN__)
  if (channel == INT0) {
...
#endif
#if SIMUL_EXT_USE_INT1 || defined(__DOXYGEN__)
  if (channel == INT1) {
    if (edge == EXT_CH_MODE_RISING_EDGE) {
      EICRA |= (1 << 2);
      EICRA |= (1 << 3);
...
#endif
#if SIMUL_EXT_USE_INT2 || defined(__DOXYGEN__)
  if (channel == INT2) {
    if (edge == EXT_CH_MODE_RISING_EDGE) {
      EICRA |= (1 << 4);
      EICRA |= (1 << 5);
...
#endif
#if SIMUL_EXT_USE_INT3 || defined(__DOXYGEN__)
  if (channel == INT3) {
    if (edge == EXT_CH_MODE_RISING_EDGE) {
      EICRA |= (1 << 6);
      EICRA |= (1 << 7);
...
#endif
#if SIMUL_EXT_USE_INT4 || defined(__DOXYGEN__)
  if (channel == INT4) {
    if (edge == EXT_CH_MODE_RISING_EDGE) {
      EICRB |= (1 << 0);
      EICRB |= (1 << 1);
...
#endif
#if SIMUL_EXT_USE_INT5 || defined(__DOXYGEN__)
  if (channel == INT5) {
    if (edge == EXT_CH_MODE_RISING_EDGE) {
      EICRB |= (1 << 2);
      EICRB |= (1 << 3);
